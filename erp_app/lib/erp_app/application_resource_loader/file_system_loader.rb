		...
		class FileSystemLoader 
		  ...
		  def locate_resources_in_engines(resource_type)
  		  dirs = [Rails.root]
  			...
  			morefiles = []
  			dirs.each do |engine_dir|
    			if File.exists? File.join(engine_dir,"/public/#{resource_type}/erp_app/shared/",@app_name)
    		    application_path = File.join(engine_dir,"/public/#{resource_type}/erp_app/shared/",@app_name,"*.#{(resource_type == 'javascripts') ? 'js' : 'css'}")
    		    morefiles = morefiles | Dir.glob(application_path)      			
    		  end
        end
		    morefiles = morefiles.collect{|file| File.basename(file)}
  			morefiles.map!{|file| "/erp_app/shared/#{@app_name}/#{file}"}
