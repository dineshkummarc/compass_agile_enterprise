class ErpApp::Desktop::Knitkit::WebsiteNavController < ErpApp::Desktop::Knitkit::BaseController
  ...
  def add_menu_item
    ...
    url = params[:url]
    if(params[:link_to_section] == 'yes')
      website_section = WebsiteSection.find(params[:section_id])
      website_nav_item.url = website_section.permalink.to_s
      website_nav_item.linked_to_item = website_section
      url = "http://#{website_nav.website.hosts.first.host}/" + website_section.permalink
    else
    ...
    if website_nav_item.save
      ...
      result[:success] = true
      result[:node] = {:text => params[:title], :url => url, :websiteNavItemId => website_nav_item.id, :iconCls => 'icon-document', :isWebsiteNavItem => true, :leaf => true, :children => []}
    else
      result[:success] = false
    end
