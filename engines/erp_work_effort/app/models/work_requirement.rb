class WorkRequirement < ActiveRecord::Base
  ...
  has_many    :work_requirement_work_effort_status_types,
    :dependent => :destroy
  has_many    :work_effort_status_types,
    :through => :work_requirement_work_effort_status_types

  #TODO: refactor cost needs to subclassed for hca
  belongs_to :projected_cost, :class_name => "UserApps::Hca::Cost", :foreign_key => 'cost_id'

  #TODO: refactor cost needs to subclassed for hca
  #for rooms
  belongs_to :room,
             :class_name => "UserApps::Hca::Room",
             :foreign_key => "work_requirement_record_id"
  ...
  def initial_work_effort_status_type
    ...
    unless status_rel.nil?
      status_rel.work_effort_status_type
    else
      raise "No Initial Status Set For Requirement"
    end
  end
  ...
  def create_work_efforts(effort_class, initial_status_type=nil)
    ...
    if initial_status_type.nil?
      parent_effort.start(initial_work_effort_status_type)
    else
      parent_effort.start(initial_status_type)
    end
  ...
  def get_initial_status_type
    status_rel = work_requirement_work_effort_status_types.find(:first, :conditions => 'is_initial_status = 1')
    status_rel
  end
end
