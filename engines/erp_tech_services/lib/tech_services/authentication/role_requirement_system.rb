...
module TechServices::Authentication::RoleRequirementSystem
  def self.included(klass)
    klass.send :class_inheritable_array, :role_requirements
    klass.send :include, TechServices::Authentication::RoleSecurityInstanceMethods
    klass.send :extend, TechServices::Authentication::RoleSecurityClassMethods
  ...
  module TechServices::Authentication::RoleSecurityClassMethods
      ...
      self.role_requirements.each{| role_requirement|
        ...
        roles.each { |role|
         ...
         #   passed=true
          #end
          if(user!=nil)
            passed=user.contains_role?(role)
  ...
  module TechServices::Authentication::RoleSecurityInstanceMethods
    def self.included(klass)
      raise "Because role_requirement extends acts_as_authenticated, You must include AuthenticatedSystem first before including RoleRequirementSystem!" unless klass.included_modules.include?(TechServices::Authentication::AuthenticatedSystem)
    end
