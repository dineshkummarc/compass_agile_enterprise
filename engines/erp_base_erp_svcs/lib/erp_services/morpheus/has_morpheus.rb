  ...
  module Morpheus
    module HasMorpheus
      ...
      module ClassMethods
        ...
        def has_morpheus(name, options = {})
          morpheus_class_name = self.name
          ...
          extend ErpServices::Morpheus::HasMorpheus::SingletonMethods
          include ErpServices::Morpheus::HasMorpheus::InstanceMethods
          ...
          options[:models].each do |model|
            model.to_s.classify.constantize.class_exec do
              ...
              has_many morpheus_class_name.underscore.pluralize.to_sym,
                :class_name => "#{morpheus_class_name}",
                :finder_sql => 'SELECT DISTINCT target.* from '+morpheus_class_name.tableize+' target join '+
                options[:through].to_s+' join_table on join_table.'+morpheus_class_name.underscore.singularize+'_id = target.id '+
                'and join_table.'+name.to_s.singularize+'_type = \'#{morpheus_name}\' and '+
                'join_table.'+name.to_s.singularize+'_id = #{id}'

              #we want to save the relantionships when the model is saved
              before_save do |record|
                record.send(morpheus_class_name.underscore.pluralize).each do |reln_record|
                  ...
                  db_result = ActiveRecord::Base.connection.select_all("SELECT count(*) as num_rows FROM #{options[:through]}
                                                    ...
                                                    and #{morpheus_class_name.underscore.singularize}_id = #{reln_record.id}")
...
ActiveRecord::Base.send(:include, ErpServices::Morpheus::HasMorpheus)