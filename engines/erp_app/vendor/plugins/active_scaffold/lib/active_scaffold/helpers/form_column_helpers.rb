  ...
  module Helpers
    ...
    module FormColumnHelpers
      ...
      def active_scaffold_input_options(column, scope = nil)
        ...
        { :name => name, :class => "#{column.name}-input tiny_mce", :id => id_control}
      end
      ...
      def active_scaffold_input_textarea(column, options)
        text_area(:record, column.name, options.merge(:cols => column.options[:cols], :class => :tiny_mce, :rows => column.options[:rows]))
      end
