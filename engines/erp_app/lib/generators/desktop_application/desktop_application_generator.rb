class DesktopApplicationGenerator < Rails::Generator::NamedBase
  ...
  def initialize(runtime_args, runtime_options = {})
    ...
    @icon            = runtime_args[2]
    @erp_application = runtime_args[3].blank? ? 'erp_app' : runtime_args[3]
  end
  ...
  def manifest
    record do |m|
      ...
      #Controller
      m.directory "vendor/plugins/#{@erp_application}/app/controllers/erp_app/desktop/#{file_name}"
      ...
      #Migration
      m.migration_template "migrate/migration_template.rb", "vendor/plugins/#{@erp_application}/db/data_migrations/", {:migration_file_name => "create_desktop_app_#{file_name}"}
