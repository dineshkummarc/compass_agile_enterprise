module ErpApp
  module Widget
    module ShoppingCart
      module Helpers
        class Order
          ...
          delegate :params, :session, :request, :logger, :authenticated?, :current_user, :to => :widget
          ...
          def get_order(create=nil)
            ...
            unless session['order_txn_id'].nil?
              order = OrderTxn.find(:first, :conditions => ['id = ?',session['order_txn_id']])
              if order.nil?
                order = create_order
              end
            else
          ...
          def complete_order(params, charge_credit_card=true)
            ...
            if charge_credit_card
              ...
              result = financial_txn.purchase(
                ...
                ActiveMerchantGatewayWrappers::BrainTreeGatewayWrapper,
          ...
          def create_order
            ...
            setup_biz_txn_party_roles(order) if self.authenticated?
            order
          end

          #sets up buyer party role
          def setup_biz_txn_party_roles(order)
            ...
            tpr.biz_txn_party_role_type = buyer_role
            tpr.save
          end
    ...
    end#end ShoppingCart module
  end#end Widget module
end#end ErpApp module
